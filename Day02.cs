public class Day02 : IDay
{
    ...
    public string Part1() => $"Test: {Impl1(Test1)} (verwacht 8)\nReal: {Impl1(Input)}";

    public string Part2() => $"Test: {Impl2(Test1)} (verwacht 2286)\nReal: {Impl2(Input)}";
    ...
    private string Impl1(string input)
    {
        ...
        foreach (var game in games)
        {
            ...
            var gameId = int.Parse(parts[0].Substring("Game ".Length));
            var subsets = parts[1].Split("; ");
            var ok = true;
            foreach (var subset in subsets)
            {
                var (blue, red, green) = CountSubset(subset);
                ok = ok && blue <= 14 && red <= 12 && green <= 13;
            }
            if (ok)
            {
                total += gameId;
    ...
    private string Impl2(string input)
    {
        ...
        foreach (var game in games)
        {
            ...
            var power = blueMax * redMax * greenMax;
            total += power;
        }
        return total.ToString();
    }

    private (int Blue, int Red, int Green) CountSubset(string subset)
    {
        var blue = 0;
        var red = 0;
        var green = 0;
